...
html_files = ['index.html', 'profile.html']
base_dir = '/Users/adeningwerson/Desktop/Tennis-Landing-Page'

local_function = """
// LOCAL RAQGEN COMPUTE ENGINE
function computeRaqGenLocal(p) {
...
for f in html_files:
    ...
    if "computeRaqGenLocal" not in content:
        content = content.replace('</script>\n</body>', '\n' + local_function + '\n</script>\n</body>')
        
    if f == 'index.html':
        old_fetch = """            const res = await fetch('/api/raqgen', {
                method: 'POST',
        ...
        content = content.replace(old_fetch, replacement_index.strip())
    ...
    if f == 'profile.html':
        old_fetch = """            const res = await fetch('/api/raqgen', {
            ...
            const data = await res.json();"""
        content = content.replace(old_fetch, replacement_profile.strip())
        
    with open(filepath, 'w') as file:
        file.write(content)

print("Done carefully replacing.")
